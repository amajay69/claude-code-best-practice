...
// Demo data for BizConnect. Exposes:
//   seed()        — wipe everything and insert fresh demo data (CLI: `npm run seed`)
//   seedIfEmpty() — insert demo data ONLY if the database has no members (safe
//                   for first-boot seeding in production; never wipes real data)
...
// Insert the demo graph (groups → members → posts → listings → responses).
// Assumes the relevant tables are empty.
function populate() {
  ...
  // Splice the shared demo password hash in before group_id to match columns.
  const memberIds = members.map(([name, biz, cat, email, groupId]) =>
    Number(insertMember.run(name, biz, cat, email, demoHash, groupId).lastInsertRowid)
  );
  ...
  return { groups: groups.length, members: members.length, posts: posts.length, listings: listings.length, responses: responses.length };
}

// Destructive: clear all data, then repopulate. Used by `npm run seed`.
function seed() {
  init();
...
// Non-destructive: only seed when the members table is empty. Safe to call on
// every server boot (used for first-deploy seeding behind SEED_ON_EMPTY).
...
module.exports = { seed, seedIfEmpty, DEMO_PASSWORD };

// CLI entry point: `node seed.js` / `npm run seed`.
if (require.main === module) {
  const counts = seed();
  console.log(
    `Seeded ${counts.groups} groups, ${counts.members} members, ${counts.posts} posts, ` +
    `${counts.listings} listings, ${counts.responses} responses.`
  );
